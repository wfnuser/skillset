...
方法页必须体现关键机制，不能只有流程概述或训练阶段名称。根据论文选择核心公式、算法伪代码或结构图，讲清输入与输出、关键运算，以及损失或奖励如何驱动学习；简要解释变量并关联原图。优先展示本文新增的设计，不为凑公式堆放通用算法，也不补写原文未给出的实现细节。精简文字时保留这些方法证据。
...
- **机构名称使用原名**：中国机构使用中文名称或通行简称（如“中科院”）；非中国机构保留其官方原名或通行简称，不翻译成中文。此规则同时适用于页脚、正文和独立讲解网页。
